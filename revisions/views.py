...
from .mixins import QuerysetFilterMixin,CreateMixin, UpdateMixin, DeleteMixin
...
class StandardPpeListView(QuerysetFilterMixin,ListView):
    model = StandardPpe
...
class ManufacturerListView(QuerysetFilterMixin,ListView):
    model = Manufacturer
...
class LifetimeOfPpeListView(QuerysetFilterMixin,ListView):
    model = LifetimeOfPpe
    template_name = 'lifetime_of_ppe_list.html'
    context_object_name = 'lifetimes_of_ppe'
    success_url = reverse_lazy('revision_home')
    search_fields = ['manufacturer__name','material_type__name','lifetime_use_years','lifetime_manufacture_years']
    # def get_queryset(self):
    #     queryset = super().get_queryset()
    #     query = self.request.GET.get('q')
    #     if query:
...
class TypeOfPpeListView(QuerysetFilterMixin,ListView):
    model = TypeOfPpe
    template_name = 'type_of_ppe_list.html'
    context_object_name = 'types_of_ppe'
    success_url = reverse_lazy('revision_home')
    search_fields = ['group_type_ppe','price']
    # def get_queryset(self):
    #     queryset = super().get_queryset()
    #     query = self.request.GET.get('q')
    #     if query:
...
class RevisionDataListView(QuerysetFilterMixin,ListView):
    model = RevisionData
